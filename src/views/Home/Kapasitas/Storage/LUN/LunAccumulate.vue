<template>
  <div class="container" v-if="showAcc">
    <v-card-title><h1 class="mb-4">Akumulasi Lun: {{this.id_lun.n_stor_lun}}</h1></v-card-title>
    <hr />
    <v-menu :rounded="rounded" offset-y>
      ...
      <v-list>
        <v-list-item>
          <v-list-item-title>
            <download-excel :data="lunaccs" name="lunaccs.xls">
              Export To XLS
            </download-excel>
          </v-list-item-title>
        </v-list-item>
        <v-list-item>
          <v-list-item-title>
            <download-excel :data="lunaccs" type="csv" name="lunaccs.csv">
              Export To CSV
            </download-excel>
          </v-list-item-title>
        </v-list-item>
    ...
    <lun-accumulate-table 
      v-bind:lunaccs="lunaccs" 
      v-bind:loading="loading"
      v-bind:menuauth="menuauth"/>
    <lun-accumulate-form
      ref="lunaccform"
      @edit-lun-accumulate="editLunAccumulate"
      @add-lun-accumulate="addLunAccumulate"
      @del-lun-accumulate="deleteLunAccumulate"
    />
  </div>
</template>
<script>
...
import LunAccumulateTable from "./LunAccumulateTable.vue";
import LunAccumulateForm from "./LunAccumulateForm.vue";
...
export default {
  ...
  methods: {
    openAddDialog() {
      this.$refs.lunaccform.openDialogAdd(this.id_lun);
    },
    openEditDialog(data) {
      this.$refs.lunaccform.openDialogEdit(data);
    },
    openDeleteDialog(data) {
      this.$refs.lunaccform.openDialogDelete(data);
    },
    editLunAccumulate(id, accumulate) {
      this.loading=true;
      let self = this;
      let data = {
      ...
      let options = {
        method: "PUT",
        url: `${process.env.VUE_APP_API_NIST}/storage/lun/accumulate`,
        params: { id: id },
    ...
    addLunAccumulate(accumulate) {
      ...
      formData.append("c_stor_lun", accumulate.c_stor_lun);
    ...
    populateLunAccumulate() {
      ...
      axios({
        method: "GET",
        url: `${process.env.VUE_APP_API_NIST}/storage/lun/accumulate`,
        params: { id: this.id_lun.c_stor_lun },
      })
        .then(function (response) {
    ...
    refresh(){
      this.populateLunAccumulate();
    },
    back(){
      this.showAcc = false;
      this.$emit("close-lun-accumulate");
    },
