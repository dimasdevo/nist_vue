<template>
  <div class="cluster-form">
          ...
          <v-card-text>
            ...
            <v-data-table :headers="headers" :items="apps_dns">
              <template v-slot:[`item.action`]="{ item }">
                <v-icon color="yellow" @click="openEditDNSDialog(item)"
                  >mdi-pencil</v-icon
                >
    ...
    <vm-dns-form ref="vmdnsform" @add-vm-dns="addVmDns" @edit-vm-dns="editVmDns"/>
...
export default {
  ...
  methods: {
      ...
      if (this.$refs.form.validate()) {
        ...
        switch (this.type) {
          case "edit":
            ...
            this.$emit("edit-vm", this.vm.c_server_vm, this.vm, this.apps_dns);
            break;
    ...
    openEditDNSDialog(item) {
      let data = Object.assign({},item)
      this.$refs.vmdnsform.openDialogEdit(data);
    },
    deleteItemConfirm() {
      this.snackbar = true;
      this.text = "Succesfull deleting vm";
    ...
    editVmDns(dnsdata,id) {
      console.log(dnsdata);
      console.log(id);
      this.apps_dns = this.apps_dns.map(function(dns) { 
          return dns.c_server_vmdns === id ? dnsdata : dns;
      });
      console.log(this.apps_dns)
    },
